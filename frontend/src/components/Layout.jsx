...
export default function Layout({ children, currentStep, onBack, onNavigate, admin, user: propUser, onLogout, notification, onClearNotification }) {
  const currentUser = propUser || admin;
  ...
  const activeNav = currentStep;

  const getUserBadge = () => {
    if (userRole === 'DOCTOR') return 'Attending Doctor';
    if (userRole === 'PATIENT') return currentUser?.hospitalId || 'Patient Portal';
    if (userRole === 'RECEPTIONIST') return 'Desk Receptionist';
    return 'Desk Admin';
  };

  const getUserName = () => {
    return currentUser?.fullName || currentUser?.name || currentUser?.username || 'User';
  };

  const getAvatarInitials = () => {
    const name = getUserName();
    if (userRole === 'DOCTOR') return 'DR';
    if (userRole === 'PATIENT') return 'PT';
    return name ? name.substring(0, 2).toUpperCase() : 'OP';
  };

  const isOcrFlow = ['upload', 'scanning', 'review', 'success'].includes(currentStep);
      ...
      <header className="fixed top-0 w-full z-50 glass-header px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <nav className="flex flex-wrap gap-2">
            {navItems.map((item) => {
              const isActive = activeNav === item.id || (item.id === 'upload' && isOcrFlow);
              return (
                <button
                  key={item.id}
                  type="button"
          ...
          {currentUser && (
            <div className="flex items-center gap-3 pr-2 border-r border-outline-variant/10">
              ...
              <div className="hidden lg:flex flex-col text-left">
                <span className="text-xs font-bold text-on-surface leading-none">{getUserName()}</span>
                <span className="text-[9px] font-semibold text-on-surface-variant/60 uppercase tracking-widest mt-0.5">{getUserBadge()}</span>
              </div>
            </div>
          )}
          ...
          <aside className="hidden md:flex flex-col gap-6 w-80 py-8">
            ...
            <div className="bg-primary/5 rounded-2xl p-6 border border-primary/10">
              ...
              <p className="text-xs text-primary/80 leading-relaxed">
                All Aadhaar data is encrypted and processed securely. Biometric data is not stored locally.
              </p>
            </div>
          </aside>
        )}
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-on-surface-variant/60 text-[11px] uppercase tracking-widest font-semibold max-w-md">
              © 2026 Ramaiah Memorial Hospital. All medical data processed securely under HIPAA guidelines.
            </p>
          </div>
